...
Option::Option(ccMenuCallback _firstSel)
{
	...
	m_BG->getTexture()->setAliasTexParameters();
	m_BG->setScale(0.7);
	this->addChild(m_BG);

	m_Bgm = MenuItemImage::create("Images/Icon_BGM_Pop.png", 
								  "Images/Icon_BGM_Push.png", 
								  CC_CALLBACK_0(Option::Set_BGM, this));
	m_Bgm->setPosition((m_BG->getContentSize().width * 0.5f) - 100, 
						m_BG->getContentSize().height * 0.5f);
	((Sprite*)m_Bgm->getNormalImage())->getTexture()->setAliasTexParameters();
	((Sprite*)m_Bgm->getSelectedImage())->getTexture()->setAliasTexParameters();
	m_Bgm->setScale(2.0f);

	m_Sound = MenuItemImage::create("Images/Icon_SoundEffect_Pop.png", 
									"Images/Icon_SoundEffect_Push.png", 
									CC_CALLBACK_0(Option::Set_Sound, this));
	m_Sound->setPosition((m_BG->getContentSize().width * 0.5f) + 100, 
						  m_BG->getContentSize().height * 0.5f);
	m_Sound->setScale(2.5f);

	m_Close = MenuItemImage::create("Images/Button_Close_Pop.png", 
									"Images/Button_Close_Push.png", 
									_firstSel);
	m_Close->setPosition(m_BG->getContentSize().width * 0.92f, 
						 m_BG->getContentSize().height * 0.9f);
